...
let creatorAuthorImage = null;
// ============================================
// KOTOBA V5.1 — VISUELS DYNAMIQUES
// IMPORTANT : aucun style ne contient de citation ou de personnage
// pré-enregistré. La seule photo affichée sur une carte est celle
// fournie pour la citation actuelle par l'utilisateur.
// ============================================
...
function drawAuthorPhoto(ctx, img, x, y, size, borderColor){
  ctx.save();
  ...
  if (img && img.complete && img.naturalWidth) {
    ...
    const g=ctx.createLinearGradient(x,y,x+size,y+size); g.addColorStop(0,borderColor); g.addColorStop(1,'#101522');
    ctx.fillStyle=g; ctx.fillRect(x,y,size,size);
    ctx.fillStyle='#fff'; ctx.font='900 44px Inter,Arial'; ctx.textAlign='center';
    ctx.fillText((creatorQuote?.character||'?').trim().charAt(0).toUpperCase(),x+size/2,y+size/2+15); ctx.textAlign='left';
  }
  ctx.restore();
  ctx.save(); ctx.strokeStyle=borderColor; ctx.lineWidth=5; ctx.beginPath(); ctx.arc(x+size/2,y+size/2,size/2+3,0,Math.PI*2); ctx.stroke(); ctx.restore();
}
function getAuthorImageForQuote(q){ return q && q.authorImage ? q.authorImage : creatorAuthorImage; }
...
function drawNeonCard(ctx,q,s,W,H){
  ...
  drawAuthorPhoto(ctx,getAuthorImageForQuote(q),W-215,225,105,s.accent);
  ...
  const size=fitFont(ctx,q.text,W-300,58,32,"700");ctx.font=`700 ${size}px Inter,Arial,sans-serif`;ctx.fillStyle=s.text;
  const lines=wrapText(ctx,q.text,W-300);let y=485-(lines.length-1)*30;lines.forEach(line=>{ctx.fillText(line,125,y);y+=size*1.27;});
...
function drawSakuraCard(ctx,q,s,W,H){
  ...
  const size=fitFont(ctx,q.text,W-300,57,31,"600","Georgia, serif");ctx.font=`600 ${size}px Georgia,serif`;ctx.fillStyle=s.text;
  const lines=wrapText(ctx,q.text,W-300);let y=485-(lines.length-1)*29;lines.forEach(line=>{ctx.fillText(line,125,y);y+=size*1.32;});
...
function drawMangaCard(ctx,q,s,W,H){
  ...
  const size=fitFont(ctx,q.text,W-300,58,31,"800");ctx.font=`800 ${size}px Inter,Arial,sans-serif`;ctx.fillStyle=s.text;
  const lines=wrapText(ctx,q.text,W-300);let y=485-(lines.length-1)*29;lines.forEach(line=>{ctx.fillText(line,125,y);y+=size*1.27;});
...
function getCreatorFormQuote(){
  const text = document.getElementById("creatorText")?.value.trim() || "";
  const character = document.getElementById("creatorCharacter")?.value.trim() || "";
  const series = document.getElementById("creatorSeries")?.value.trim() || "";
  return {
    id: "custom-" + Date.now(),
    text: text || "Ta citation apparaîtra ici…",
    character: character || "Auteur inconnu",
    series: series || "",
    authorImage: creatorAuthorImage,
    category: "Motivation",
    lang: "Français"
  };
}

function syncCreatorFromInputs(){
  creatorQuote = getCreatorFormQuote();
  const downloadBtn=document.getElementById("creatorDownloadBtn");
  if(downloadBtn){ downloadBtn.disabled=!creatorAuthorImage; downloadBtn.textContent=creatorAuthorImage?"Télécharger PNG":"Télécharger PNG — ajoute la photo de l’auteur"; }
  drawCreatorCard();
}

function initCreator(){
  const text = document.getElementById("creatorText");
  const character = document.getElementById("creatorCharacter");
  const series = document.getElementById("creatorSeries");
  const defaultQuote = quotes[0] || {text:"", character:"", series:""};

  // Préremplit une première citation, mais l'utilisateur peut tout remplacer.
  if (text && !text.value) text.value = defaultQuote.text || "";
  if (character && !character.value) character.value = defaultQuote.character || "";
  if (series && !series.value) series.value = defaultQuote.series || "";

  [text, character, series].forEach(el => {
    if (el) el.addEventListener("input", syncCreatorFromInputs);
  });

  const photoInput=document.getElementById("creatorAuthorPhoto");
  const photoPreview=document.getElementById("creatorAuthorPhotoPreview");
  const photoHint=document.getElementById("creatorAuthorPhotoHint");
  const photoClear=document.getElementById("creatorAuthorPhotoClear");
  if(photoInput){
    photoInput.addEventListener("change",()=>{
      const file=photoInput.files?.[0]; if(!file) return;
      const reader=new FileReader();
      reader.onload=()=>{
        const img=new Image(); img.onload=()=>{ creatorAuthorImage=img; if(photoPreview){photoPreview.src=reader.result;photoPreview.hidden=false;} if(photoHint)photoHint.textContent="Photo chargée ✓"; if(photoClear)photoClear.hidden=false; syncCreatorFromInputs(); };
        img.src=reader.result;
      };
      reader.readAsDataURL(file);
    });
  }
  if(photoClear){ photoClear.addEventListener("click",()=>{ creatorAuthorImage=null; if(photoInput)photoInput.value=""; if(photoPreview){photoPreview.src="";photoPreview.hidden=true;} if(photoHint)photoHint.textContent="Ajoute la photo de la personne/personnage cité. Elle sera intégrée à la carte."; photoClear.hidden=true; syncCreatorFromInputs(); }); }
